...
from it_module.it.doctype.it_asset.it_asset import get_asset_shift_factors_map
from it_module.it.doctype.it_asset.depreciation import is_last_day_of_the_month


class ITAssetShiftAllocation(Document):
	def after_insert(self):
		self.fetch_and_set_depr_schedule()

	def validate(self):
		self.asset_doc = frappe.get_doc("IT Asset", self.asset)
	...
	def fetch_and_set_depr_schedule(self):
		...
		if not any(fb.get("shift_based") for fb in self.asset_doc.finance_books):
			frappe.throw(_("IT Asset {0} is not using shift based depreciation").format(self.asset))
