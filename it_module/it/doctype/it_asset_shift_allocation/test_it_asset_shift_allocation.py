...
class TestITAssetShiftAllocation(FrappeTestCase):
	@classmethod
	def setUpClass(cls):
	...
	def test_it_asset_shift_allocation(self):
		asset = create_asset(
		...
		it_asset_shift_allocation = frappe.get_doc(
			{"doctype": "Asset Shift Allocation", "asset": asset.name}
		).insert()

		schedules = [
			[cstr(d.schedule_date), d.depreciation_amount, d.accumulated_depreciation_amount, d.shift]
			for d in it_asset_shift_allocation.get("depreciation_schedule")
		]

		self.assertEqual(schedules, expected_schedules)

		it_asset_shift_allocation = frappe.get_doc("Asset Shift Allocation", it_asset_shift_allocation.name)
		it_asset_shift_allocation.depreciation_schedule[4].shift = "Triple"
		it_asset_shift_allocation.save()
		...
		it_asset_shift_allocation.submit()
