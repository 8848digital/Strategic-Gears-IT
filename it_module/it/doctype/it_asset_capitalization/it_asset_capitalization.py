...
import erpnext
from it_module.it.doctype.it_asset.it_asset import get_asset_value_after_depreciation
...
class ITAssetCapitalization(StockController):
	def validate(self):
	...
	def on_trash(self):
		frappe.db.set_value("IT Asset", self.target_asset, "capitalized_in", None)
		super().on_trash()

	def cancel_target_asset(self):
		if self.entry_type == "Capitalization" and self.target_asset:
			asset_doc = frappe.get_doc("IT Asset", self.target_asset)
	...
	def get_gl_entries_for_consumed_asset_items(self, gl_entries, target_account, target_against, precision):
		...
		for item in self.asset_items:
			asset = frappe.get_doc("IT Asset", item.asset)
	...
	def create_target_asset(self):
		...
		asset_doc = frappe.new_doc("IT Asset")
		...
		frappe.msgprint(
			_("Asset {0} has been created. Please set the depreciation details if any and submit it.").format(
				get_link_to_form("IT Asset", asset_doc.name)
			)
		)
...
def get_target_asset_details(asset=None, company=None):
	...
	if asset:
		asset_details = frappe.db.get_value("IT Asset", asset, ["asset_name", "item_code"], as_dict=1)
...
def get_consumed_asset_details(args):
	...
	if args.asset:
		asset_details = frappe.db.get_value(
			"IT Asset", args.asset, ["asset_name", "item_code", "item_name"], as_dict=1
		)
		if not asset_details:
