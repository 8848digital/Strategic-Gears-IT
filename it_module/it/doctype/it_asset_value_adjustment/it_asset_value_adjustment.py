...
from it_module.it.doctype.it_asset.depreciation import get_depreciation_accounts


class ITAssetValueAdjustment(Document):
	def validate(self):
		self.validate_date()
	...
	def validate_date(self):
		asset_purchase_date = frappe.db.get_value("IT Asset", self.asset, "purchase_date")
		if getdate(self.date) < getdate(asset_purchase_date):
			frappe.throw(
				_("IT Asset Value Adjustment cannot be posted before Asset's purchase date <b>{0}</b>.").format(
