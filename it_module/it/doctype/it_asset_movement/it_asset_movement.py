...
class ITAssetMovement(Document):
	...
	def validate_asset(self):
		for d in self.assets:
			status, company = frappe.db.get_value("IT Asset", d.asset, ["status", "company"])
			if self.purpose == "Transfer" and status in ("Draft", "Scrapped", "Sold"):
				frappe.throw(_("{0} asset cannot be transferred").format(status))

			if company != self.company:
				frappe.throw(_("IT Asset {0} does not belong to company {1}").format(d.asset, self.company))
	...
	def validate_location(self):
		for d in self.assets:
			if self.purpose in ["Transfer", "Issue"]:
				current_location = frappe.db.get_value("IT Asset", d.asset, "location")
				if d.source_location:
					if current_location != d.source_location:
						frappe.throw(
							_("IT Asset {0} does not belongs to the location {1}").format(
			...
			if self.purpose == "Receipt":
				if not (d.source_location) and not (d.target_location or d.to_employee):
					...
					if d.from_employee and not d.target_location:
						...
						frappe.throw(
							_(
								"IT Asset {0} cannot be received at a location and given to an employee in a single movement"
							).format(d.asset)
						)
	...
	def validate_employee(self):
		for d in self.assets:
			if d.from_employee:
				current_custodian = frappe.db.get_value("IT Asset", d.asset, "custodian")

				if current_custodian != d.from_employee:
					frappe.throw(
						_("IT Asset {0} does not belongs to the custodian {1}").format(d.asset, d.from_employee)
					)
	...
	def set_latest_location_and_custodian_in_asset(self):
		...
		for d in self.assets:
			...
			latest_movement_entry = frappe.db.sql(
				...
				FROM `tabIT Asset Movement Item` asm_item, `tabIT Asset Movement` asm
				WHERE
			...
			frappe.db.set_value("IT Asset", d.asset, "location", current_location)
			frappe.db.set_value("IT Asset", d.asset, "custodian", current_employee)
