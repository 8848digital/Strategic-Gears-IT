...
def get_depreciable_assets(date):
	a = frappe.qb.DocType("IT Asset")
	ds = frappe.qb.DocType("IT Depreciation Schedule")
...
def set_depr_entry_posting_status_for_failed_assets(failed_asset_names):
	for asset_name in failed_asset_names:
		frappe.db.set_value("IT Asset", asset_name, "depr_entry_posting_status", "Failed")
...
def notify_depr_entry_posting_error(failed_asset_names, error_log_names):
	...
	asset_links = get_comma_separated_links(failed_asset_names, "IT Asset")
...
def scrap_asset(asset_name):
	...
	if asset.docstatus != 1:
		frappe.throw(_("IT Asset {0} must be submitted").format(asset.name))
	elif asset.status in ("Cancelled", "Sold", "Scrapped", "Capitalized", "Decapitalized"):
		frappe.throw(_("IT Asset {0} cannot be scrapped, as it is already {1}").format(asset.name, asset.status))
	...
	frappe.db.set_value("IT Asset", asset_name, "disposal_date", date)
	frappe.db.set_value("IT Asset", asset_name, "journal_entry_for_scrap", je.name)
	asset.set_status("Scrapped")

	frappe.msgprint(_("IT Asset scrapped via Journal Entry {0}").format(je.name))
...
def modify_depreciation_schedule_for_asset_repairs(asset):
	asset_repairs = frappe.get_all(
		"IT Asset Repair", filters={"asset": asset.name}, fields=["name", "increase_in_asset_life"]
	)

	for repair in asset_repairs:
		if repair.increase_in_asset_life:
			asset_repair = frappe.get_doc("IT Asset Repair", repair.name)
...
def get_value_after_depreciation_on_disposal_date(asset, disposal_date, finance_book=None):
	asset_doc = frappe.get_doc("IT Asset", asset)
