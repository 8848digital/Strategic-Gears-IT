...
from it_module.it.doctype.it_asset_category.it_asset_category import get_asset_category_account
from erpnext.controllers.accounts_controller import AccountsController


class ITAsset(AccountsController):
	def validate(self):
	...
	def on_cancel(self):
		...
		make_reverse_gl_entries(voucher_type="IT Asset", voucher_no=self.name)
	...
	def validate_item(self):
		...
		if not item:
			...
			frappe.throw(_("Item {0} must be a Fixed IT Asset Item").format(self.item_code))
	...
	def validate_cost_center(self):
		if self.cost_center:
			...
			if not frappe.get_cached_value("Company", self.company, "depreciation_cost_center"):
				frappe.throw(
					_(
						"Please set a Cost Center for the IT Asset or set an IT Asset Depreciation Cost Center for the Company {}"
	...
	def validate_gross_and_purchase_amount(self):
		...
		if self.gross_purchase_amount and self.gross_purchase_amount != self.purchase_receipt_amount:
			error_message = _(
				"Gross Purchase Amount should be <b>equal</b> to purchase amount of one single IT Asset."
			)
			error_message += "<br>"
			error_message += _("Please do not book expense of multiple assets against one single IT Asset.")
	...
	def _add_depreciation_row(self, schedule_date, depreciation_amount, finance_book, schedule_idx):
		if finance_book.shift_based:
			shift = (
				...
				else frappe.get_cached_value("IT Asset Shift Factor", {"default": 1}, "shift_name")
			)
		else:
			shift = None
	...
	def validate_cancellation(self):
		...
		if self.status not in ("Submitted", "Partially Depreciated", "Fully Depreciated"):
			frappe.throw(_("IT Asset cannot be cancelled, as it is already {0}").format(self.status))

	def cancel_movement_entries(self):
		movements = frappe.db.sql(
			"""SELECT asm.name, asm.docstatus
			FROM `tabIT Asset Movement` asm, `tabIT Asset Movement Item` asm_item
		...
		for movement in movements:
			movement = frappe.get_doc("IT Asset Movement", movement.get("name"))
			movement.cancel()

	def cancel_capitalization(self):
		asset_capitalization = frappe.db.get_value(
			"IT Asset Capitalization",
			{"target_asset": self.name, "docstatus": 1, "entry_type": "Capitalization"},
		)

		if asset_capitalization:
			asset_capitalization = frappe.get_doc("IT Asset Capitalization", asset_capitalization)
		...
		if not fixed_asset_account:
			frappe.throw(
				_("Set {0} in asset category {1} for company {2}").format(
					frappe.bold("Fixed IT Asset Account"),
				...
				self.get_gl_dict(
					{
						...
						"remarks": self.get("remarks") or _("Accounting Entry for IT Asset"),
...
def update_maintenance_status():
	assets = frappe.get_all(
		"IT Asset", filters={"docstatus": 1, "maintenance_required": 1, "disposal_date": ("is", "not set")}
	)

	for asset in assets:
		asset = frappe.get_doc("IT Asset", asset.name)
		if frappe.db.exists("IT Asset Repair", {"asset_name": asset.name, "repair_status": "Pending"}):
			asset.set_status("Out of Order")
		elif frappe.db.exists("IT Asset Maintenance Task", {"parent": asset.name, "next_due_date": today()}):
...
def make_post_gl_entry():
	asset_categories = frappe.db.get_all("IT Asset Category", fields=["name", "enable_cwip_accounting"])

	for asset_category in asset_categories:
		if cint(asset_category.enable_cwip_accounting):
			assets = frappe.db.sql_list(
				""" select name from `tabIT Asset`
			...
			for asset in assets:
				doc = frappe.get_doc("IT Asset", asset)
				doc.make_gl_entries()


def get_asset_naming_series():
	meta = frappe.get_meta("IT Asset")
...
def create_asset_maintenance(asset, item_code, item_name, asset_category, company):
	asset_maintenance = frappe.new_doc("IT Asset Maintenance")
...
def create_asset_repair(asset, asset_name):
	asset_repair = frappe.new_doc("IT Asset Repair")
...
def create_asset_capitalization(asset):
	asset_capitalization = frappe.new_doc("IT Asset Capitalization")
...
def create_asset_value_adjustment(asset, asset_category, company):
	asset_value_adjustment = frappe.new_doc("IT Asset Value Adjustment")
...
def transfer_asset(args):
	...
	movement_entry = frappe.new_doc("IT Asset Movement")
	...
	frappe.msgprint(
		_("IT Asset Movement record {0} created")
		.format("<a href='/app/Form/IT Asset Movement/{0}'>{0}</a>")
...
def get_item_details(item_code, asset_category, gross_purchase_amount):
	asset_category_doc = frappe.get_doc("IT Asset Category", asset_category)
	books = []
...
def make_asset_movement(assets, purpose=None):
	...
	asset_movement = frappe.new_doc("IT Asset Movement")
	asset_movement.quantity = len(assets)
	for asset in assets:
		asset = frappe.get_doc("IT Asset", asset.get("name"))
...
def is_cwip_accounting_enabled(asset_category):
	return cint(frappe.db.get_value("IT Asset Category", asset_category, "enable_cwip_accounting"))
...
def get_straight_line_or_manual_depr_amount(asset, row, schedule_idx, number_of_pending_depreciations):
	...
	# if the Depreciation Schedule is being modified after IT Asset Repair due to increase in asset life and value
	if asset.flags.increase_in_asset_life:
		return (flt(row.value_after_depreciation) - flt(row.expected_value_after_useful_life)) / (
			date_diff(asset.to_date, asset.available_for_use_date) / 365
		)
	# if the Depreciation Schedule is being modified after IT Asset Repair due to increase in asset value
	elif asset.flags.increase_in_asset_value_due_to_repair:
		return (flt(row.value_after_depreciation) - flt(row.expected_value_after_useful_life)) / flt(
			number_of_pending_depreciations
		)
	# if the Depreciation Schedule is being modified after IT Asset Value Adjustment due to decrease in asset value
...
def get_asset_shift_factors_map():
	return dict(frappe.db.get_all("IT Asset Shift Factor", ["shift_name", "shift_factor"], as_list=True))
...
@frappe.whitelist()
def splasset(asset_name, split_qty):
	asset = frappe.get_doc("IT Asset", asset_name)
...
def update_existing_asset(asset, remaining_qty):
	...
	for finance_book in asset.get("finance_books"):
		...
		frappe.db.set_value(
			"IT Asset Finance Book", finance_book.name, "value_after_depreciation", value_after_depreciation
		)
		frappe.db.set_value(
			"IT Asset Finance Book",
			finance_book.name,
			"expected_value_after_useful_life",
			expected_value_after_useful_life,
		)
...
@frappe.whitelist()
def map_asset_to_it_asset(source_name):
	source_doctype = "Asset"
	target_doctype = "IT Asset"

	

	# Generate a mapped document
	...
	if target_doc and hasattr(target_doc, 'finance_books'):
		target_doc.finance_books = []

		source_doc = frappe.get_doc(source_doctype, source_name)
		target_doc.purchase_receipt = source_doc.purchase_receipt
		target_doc.purchase_invoice = source_doc.purchase_invoice
		source_finance_books = source_doc.get('finance_books', [])
		dep = source_doc.get('schedules', [])


		if source_finance_books:
			for row in source_finance_books:
				target_doc.append('finance_books', row.as_dict()) 
		if dep:
			for row in dep:
				target_doc.append('schedules', row.as_dict()) 


		frappe.log_error(str(target_doc.finance_books), "Target Finance Books")
