...
class ITLocation(NestedSet):
	nsm_parent_field = "parent_it_location"

	def validate(self):
		self.calculate_it_location_area()

		if not self.is_new() and self.get("parent_it_location"):
			self.update_ancestor_it_location_features()

	def on_update(self):
		# super(ITLocation, self).on_update()
		NestedSet.on_update(self)

	def on_trash(self):
		NestedSet.validate_if_child_exists(self)
		update_nsm(self)
		self.remove_ancestor_it_location_features()
		# super(ITLocation, self).on_update()

	def calculate_it_location_area(self):
		features = self.get_it_location_features()
		new_area = compute_area(features)

		self.area_difference = new_area - flt(self.area)
		self.area = new_area

	def get_it_location_features(self):
	...
	def set_it_location_features(self, features):
		if not self.location:
			self.location = '{"type":"FeatureCollection","features":[]}'

		it_location = json.loads(self.location)
		it_location["features"] = features

		self.db_set("it_location", json.dumps(it_location), commit=True)

	def update_ancestor_it_location_features(self):
		self_features = set(self.add_child_property())

		for ancestor in self.get_ancestors():
			ancestor_doc = frappe.get_doc("IT Location", ancestor)
			...
			ancestor_doc.set_it_location_features(features=ancestor_features)
			ancestor_doc.db_set("area", ancestor_doc.area + self.area_difference, commit=True)

	def remove_ancestor_it_location_features(self):
		for ancestor in self.get_ancestors():
			ancestor_doc = frappe.get_doc("IT Location", ancestor)
			child_features, ancestor_features = ancestor_doc.feature_seperator(child_feature=self.name)
...
def compute_area(features):
	"""
	Calculate the total area for a set of it_location features.
...
@frappe.whitelist()
def get_children(doctype, parent=None, it_location=None, is_root=False):
	if parent is None or parent == "All IT Locations":
		parent = ""
	...
	return frappe.db.sql(
		...
		from
			`tabIT Location` comp
		where
			ifnull(parent_it_location, "")={frappe.db.escape(parent)}
		""",
		as_dict=1,
	)
...
def add_node():
	...
	if args.parent_it_location == "All IT Locations":
		args.parent_it_location = None
