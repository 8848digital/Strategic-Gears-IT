...
from erpnext.assets.doctype.it_asset_maintenance.it_asset_maintenance import calculate_next_due_date
from erpnext.stock.doctype.purchase_receipt.test_purchase_receipt import make_purchase_receipt


class TestITAssetMaintenance(unittest.TestCase):
	def setUp(self):
		set_depreciation_settings_in_company()
		create_asset_data()
		create_maintenance_team()

	def test_create_it_asset_maintenance(self):
		...
		if not frappe.db.exists("Asset Maintenance", "Photocopier"):
			it_asset_maintenance = frappe.get_doc(
				{
					"doctype": "Asset Maintenance",
					"asset_name": "Photocopier",
					"maintenance_team": "Team Awesome",
					"company": "_Test Company",
					"it_asset_maintenance_tasks": get_maintenance_tasks(),
				}
			).insert()

			next_due_date = calculate_next_due_date(nowdate(), "Monthly")
			self.assertEqual(it_asset_maintenance.it_asset_maintenance_tasks[0].next_due_date, next_due_date)

	def test_create_it_asset_maintenance_log(self):
		if not frappe.db.exists("Asset Maintenance Log", "Photocopier"):
			it_asset_maintenance_log = frappe.get_doc(
				{
					"doctype": "Asset Maintenance Log",
					"it_asset_maintenance": "Photocopier",
					"task": "Change Oil",
					"completion_date": add_days(nowdate(), 2),
					"maintenance_status": "Completed",
				}
			).insert()
		it_asset_maintenance = frappe.get_doc("Asset Maintenance", "Photocopier")
		next_due_date = calculate_next_due_date(it_asset_maintenance_log.completion_date, "Monthly")
		self.assertEqual(it_asset_maintenance.it_asset_maintenance_tasks[0].next_due_date, next_due_date)
