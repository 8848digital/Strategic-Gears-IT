...
frappe.ui.form.on("IT Asset Maintenance", {
	setup: (frm) => {
		frm.set_query("assign_to", "asset_maintenance_tasks", function (doc) {
			return {
				query: "it_module.it.doctype.it_asset_maintenance.it_asset_maintenance.get_team_members",
				filters: {
	...
	make_dashboard: (frm) => {
		if (!frm.is_new()) {
			frappe.call({
				method: "it_module.it.doctype.it_asset_maintenance.it_asset_maintenance.get_maintenance_log",
					...
					(r.message || []).forEach(function (d) {
						$(`<div class='row' style='margin-bottom: 10px;'>
							<div class='col-sm-3 small'>
								<a onclick="frappe.set_route('List', 'IT Asset Maintenance Log',
...
frappe.ui.form.on("IT Asset Maintenance Task", {
...
var get_next_due_date = function (frm, cdt, cdn) {
	...
	if (d.start_date && d.periodicity) {
		return frappe.call({
			method: "it_module.it.doctype.it_asset_maintenance.it_asset_maintenance.calculate_next_due_date",
			args: {
