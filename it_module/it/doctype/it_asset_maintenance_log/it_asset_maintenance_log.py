...
from it_module.it.doctype.it_asset_maintenance.it_asset_maintenance import calculate_next_due_date


class ITAssetMaintenanceLog(Document):
	def validate(self):
	...
	def update_maintenance_task(self):
		asset_maintenance_doc = frappe.get_doc("IT Asset Maintenance Task", self.task)
		...
		if self.maintenance_status == "Cancelled":
			...
			asset_maintenance_doc.save()
		asset_maintenance_doc = frappe.get_doc("IT Asset Maintenance", self.asset_maintenance)
		asset_maintenance_doc.save()
...
def get_maintenance_tasks(doctype, txt, searchfield, start, page_len, filters):
	asset_maintenance_tasks = frappe.db.get_values(
		"IT Asset Maintenance Task", {"parent": filters.get("asset_maintenance")}, "maintenance_task"
	)
