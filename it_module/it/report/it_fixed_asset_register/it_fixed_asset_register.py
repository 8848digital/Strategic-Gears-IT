...
from it_module.it.doctype.it_asset.it_asset import get_it_asset_value_after_depreciation
...
def execute(filters=None):
	...
	chart = (
		...
		if filters.get("group_by") not in ("IT Asset Category", "IT Location")
		else {}
	)
...
def get_data(filters):
	...
	assets_record = frappe.db.get_all("IT Asset", filters=conditions, fields=fields)
...
def get_assets_linked_to_fb(filters):
	afb = frappe.qb.DocType("IT Asset Finance Book")
...
def get_asset_depreciation_amount_map(filters, finance_book):
	...
	asset = frappe.qb.DocType("IT Asset")
	gle = frappe.qb.DocType("GL Entry")
	aca = frappe.qb.DocType("IT Asset Category Account")
...
def get_group_by_data(group_by, conditions, assets_linked_to_fb, depreciation_amount_map):
	...
	assets = frappe.db.get_all("IT Asset", filters=conditions, fields=fields)
...
def get_columns(filters):
	if filters.get("group_by") in ["IT Asset Category", "IT Location"]:
		return [
			{
